...
namespace Grading_System_Backend.Services
{
    public class SubjectService
    {
        ...
        public StudentSubjects Add(AddEditStudentSubjectDto dto)
        {
            ...
            if (studentSubject != null)  throw new Exception("there is already record");

            if (dto.Term1 > subject.MaximumDegree || dto.Term2 > subject.MaximumDegree)
                throw new Exception("Term value exceeded the maximum range");
        ...
        public List<SubjectStudentsDto> GetSubjectsWithStudents()
        {
        ...
        public StudentSubjects Update(AddEditStudentSubjectDto dto)
        {
            var studentSubjects = _unitOfWork.StudentSubjectRepo
                .getAsQuery()
                .Include(s => s.Subject)
                .FirstOrDefault(s => s.Subject.id == dto.SubjectId && s.Student.Id == dto.StudentId);
            if (studentSubjects == null) throw new Exception("Student subject is invalid");
            studentSubjects.Term1 = dto.Term1;
            studentSubjects.Term2 = dto.Term2;
            studentSubjects.Total = dto.Term1 + dto.Term2;
